...
**Supported:** single-band and multi-band GeoTIFFs (up to 4 bands, parsed into an additional `band` dimension); uncompressed and Deflate-compressed tiles
...
pushdown applies; the value column uses the COG's real data type. If the file is a JPEG2000 (`.jp2`), Eider can parse its structure with fallback heuristics.

**Not yet supported:** Unsupported bit-depths,
...
For remote STAC Search APIs, DuckDB's spatial (`lat_min`/`lon_max`) and temporal (`time_min`/`time_max`) bounding filters are aggressively pushed down to the server by translating them into STAC `bbox` and `datetime` query parameters. This ensures Eider only downloads metadata and data for STAC Items that actually intersect your query. The API response is fully paginated natively — Eider will eagerly traverse `rel:next` links to fetch the full matching collection.
...
**Not yet supported:** **multi-resolution** collections (assets
